...
void Framework::Init(int width, int height)
{
	...
	m_OldWidth = m_InitialWidth;
	m_OldHeight = m_InitialHeight;
...
void Framework::GenerateXInputEvents()
{
    XINPUT_STATE state;
    ...
    {
        // TODO: write error message for user
        return;
    }



    // TODO: Poll controller for status
    //       Create appropriate input event

    /*if (state.Gamepad.wButtons & VK_PAD_A)
    {
        InputEvent e1(DeviceType::Joypad, KeyState::Pressed, GamepadID::A)
    }*/
}
...
void Framework::ResizeWindow(int width, int height)
{
	...
	m_OldWidth = m_CurrentWidth;
	m_OldHeight = m_CurrentHeight;
...
Point Framework::GetWindowSize()
{
    return Point(m_CurrentWidth, m_CurrentHeight);
}

Point Framework::GetPreviousWindowSize()
{
	return Point(m_OldWidth, m_OldHeight);
}
    ...
    case WM_KEYDOWN:
        {
            ...
            // Event Handler
            InputEvent *ev = new InputEvent(DeviceType::Keyboard, KeyState::Pressed, (int)wParam, 0.0f, 0.0f);
            pFramework->m_pGameCore->AddEvent(ev);
        }
        return 0;
    ...
    case WM_KEYUP:
        {
            ...
            // Event Handler
            InputEvent *ev = new InputEvent(DeviceType::Keyboard, KeyState::Released, (int)wParam, 0.0f, 0.0f);
            pFramework->m_pGameCore->AddEvent(ev);
        }
        return 0;
    ...
    case WM_LBUTTONDOWN:
        {
			...
			InputEvent *ev = new InputEvent(DeviceType::Mouse, KeyState::Pressed, (int)wParam, (float)xPos, (float)yPos);
			pFramework->m_pGameCore->AddEvent(ev);
        }
        return 0;
